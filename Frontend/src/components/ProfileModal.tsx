...
export function ProfileModal({ user, open, onClose, onLogout }: ProfileModalProps) {
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('sdu_user', JSON.stringify(updatedUser));
  ...
  const handleDeleteAccount = () => {
    if (confirm('Are you sure you want to delete your account? This cannot be undone.')) {
      ...
      localStorage.removeItem('sdu_benchmarks');
      toast.success('Account deleted');
      onLogout();
    }
  };

  const savedResults = JSON.parse(localStorage.getItem('sdu_benchmarks') || '[]');
        ...
        <div className="space-y-4 py-3">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <Avatar
                className="w-20 h-20 border-2 border-red-600"
                style={{ boxShadow: '0 0 30px rgba(255, 0, 0, 0.5), 0 0 60px rgba(255, 0, 0, 0.3)' }}
              >
              ...
              {editing ? (
                <div className="space-y-2">
                  <div>
                    ...
                    <Input
                      ...
                      className="bg-black border-red-600/50 text-white text-sm h-8"
                ...
                <div className="space-y-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-white text-sm">{new Date(user.joinedDate).toLocaleDateString()}</div>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {savedResults.map((result: any, index: number) => (
                  <motion.div
                    ...
                    className="bg-black/50 border border-red-600/30 p-2 rounded flex justify-between items-center"
                  >
                    <div>
            ...
            {editing ? (
              <>
                ...
                >
                  <Button
                    onClick={handleSave}
                    className="w-full bg-red-600 hover:bg-red-700 text-white transition-all duration-300 text-sm h-9"
              ...
              <>
                ...
                >
                  <Button
                    ...
                    className="w-full bg-transparent border-2 border-red-600 text-white hover:bg-red-600/20 transition-all duration-300 text-sm h-9"
