...
export function AnalysisUpgrades({ user }: AnalysisUpgradesProps) {
  // ✅ Defensive: if user is missing, show loading
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-[#1a1a1a] border-2 border-red-600 p-6" style={{ boxShadow: '0 0 30px rgba(255,0,0,0.4), 0 0 60px rgba(255,0,0,0.2)' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={bottleneckData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} dataKey="value" label={({ name, value }) => `${name}: ${value}%`}>
      ...
      <Card className="bg-[#1a1a1a] border-2 border-red-600 p-4" style={{ boxShadow: '0 0 30px rgba(255,0,0,0.4),0 0 60px rgba(255,0,0,0.2)' }}>
        <Tabs defaultValue="reviews" className="w-full">
          <TabsList className="bg-black border border-red-600/30">
            <TabsTrigger value="results" className="data-[state=active]:bg-red-600 data-[state=active]:text-white text-gray-400">My Saved Results</TabsTrigger>
          ...
          <TabsContent value="results" className="mt-4">
            {savedResults.length === 0 ? (
              <div className="text-center text-gray-400 py-6 text-sm">No saved benchmark results yet. Run a benchmark to see results here.</div>
            ) : (
              <div className="space-y-3">
                {savedResults.map((result: any, index: number) => (
                  <motion.div key={index} className="bg-black/50 border border-red-600/30 p-3 rounded" whileHover={{ boxShadow: '0 0 20px rgba(255,0,0,0.3)' }}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="text-red-500 text-sm" style={{ fontFamily: 'Orbitron, sans-serif' }}>{result.overallScore}</div>
                    </div>
